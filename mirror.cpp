...
// printfpd("\n\r mt 5a: %02X", inportb(MIRROR_PORT) );
 if(test != inportb(MIRROR_PORT))  ret = 0;
 test = 0xA5;
 outportb(MIRROR_PORT, test);
// printfpd("\n\r mt a5: %02X", inportb(MIRROR_PORT) );
 if(test != inportb(MIRROR_PORT))  ret = 0;

#ifdef	PROG_STM2UDP
 test = 0x5A;
 outportb(MIRROR_PORT + 0x200, test);
// printfpd("\n\r mt 5a: %02X", inportb(MIRROR_PORT+ 0x200) );
 if(test != inportb(MIRROR_PORT + 0x200))  ret = 0;
 test = 0xA5;
 outportb(MIRROR_PORT + 0x200, test);
// printfpd("\n\r mt a5: %02X", inportb(MIRROR_PORT+ 0x200) );
 if(test != inportb(MIRROR_PORT + 0x200))  ret = 0;
