...
// sprintf((char*)&pOperProg->noise1, "%8d",  GetNoise1());
//  printfp1("\n\rs");
//  printfp1(OperateProg.Value);


//   printfp("\n\rf");

// printfp1("l:");
// printfpd1("L:%02X ",pOperProg->unmod0set);
// printfpd1("F:%d ", flagunmod0);
...
//} BCM_UW_WB_TRANSMITTER_MODE_ENUM;
 ...
 {//printfp(" 5");
  if(!flagunmod0){SetModuli(0, BCM_UW_WB_TRANSMITTER_MODE_ONETONE_E, 0, 0); flagunmod0 = 1;}}
 else  {//printfp(" 6"); 
 if(flagunmod0){SetModuli(0, BCM_UW_WB_TRANSMITTER_MODE_NORMAL_E, 0, 0); flagunmod0 = 0;}}
 pOperProg->unmod0get = flagunmod0 ? '1' : '0';

 if(pOperProg->unmod1set != '0')
 {//printfp(" 7");
  if(!flagunmod1){SetModuli(1, BCM_UW_WB_TRANSMITTER_MODE_ONETONE_E, 0, 0); flagunmod1 = 1;}}
 else  {//printfp(" 8"); 
 if(flagunmod1){SetModuli(1, BCM_UW_WB_TRANSMITTER_MODE_NORMAL_E, 0, 0); flagunmod1 = 0;}}
 pOperProg->unmod1get = flagunmod1 ? '1' : '0';

 if(pOperProg->bbloop0set != '0')
 {//printfp(" 9");
  if(!flagbbloop0){flagloop0710 = 1; flagbbloop0 = 1;
  SetLoopi(0, BCM_UW_LOOPBACK_MODEM_E,0, 1,0);
  }}
 else  {//printfp(" 10");
  if(flagbbloop0){ flagloop0710 = 1; flagbbloop0 = 0;
  SetLoopi(0, BCM_UW_LOOPBACK_NONE_E,0, 1,0);
  }}	 
 ...
 if(!flagbbloop1){flagloop1710 = 1; flagbbloop1 = 1;
  SetLoopi(1, BCM_UW_LOOPBACK_MODEM_E,0, 1,0);
}}
 else  {//printfp(" 12"); 

 if(flagbbloop1){ flagloop1710 = 1; flagbbloop1 = 0;
  SetLoopi(1, BCM_UW_LOOPBACK_NONE_E,0, 1,0);
 }}
 ...
 {//printfp(" 13");
  if(!flagifloop0){flagloop0710 = 1; flagifloop0 = 1;
   SetLoopRxTx(0, BCM_UW_WB_RX_ADPD0_E , 1);
  }}
 else  {//printfp(" 14"); 
 if(flagifloop0){ flagloop0710 = 1; flagifloop0 = 0;
 SetLoopRxTx(0, BCM_UW_WB_RX_ADPD0_E , 0);
 }}
 ...
 {//printfp(" 15");
  ...
  SetLoopRxTx(1, BCM_UW_WB_RX_ADPD0_E , 1);
  }}
 else  {//printfp(" 16");
  if(flagifloop1){ flagloop1710 = 1; flagifloop1 = 0;
  SetLoopRxTx(1, BCM_UW_WB_RX_ADPD0_E , 0);
  }}
...
// if(flagloop0710) 
// {
//  SetLoopRxTx(0, BCM_UW_WB_RX_0_E, 1);
